...
def view_extrato(request):
    ...
    if perido_get:
        valores = valores.filter(data__week=datetime.now().week())
...
def exportar_pdf(request):
    ...
    tmeplate_render = render_to_string(path_template, {'valores': valores})

    path_output = BytesIO()

    HTML(string=tmeplate_render).write_pdf(path_output)
